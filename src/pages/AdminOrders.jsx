...
const AdminOrders = () => {
  ...
  const addItemToManual = (item) => {
    let markupPercent = 0;
    if (selectedPlatform === "Swiggy") markupPercent = Number(config.swiggyMarkup) || 20;
    if (selectedPlatform === "Zomato") markupPercent = Number(config.zomatoMarkup) || 25;

    const platformPrice = item.price * (1 + markupPercent / 100);
    ...
    if (exists) {
      setNewOrder({
        ...
        items: newOrder.items.map((i) =>
          i.menuId === item._id ? { ...i, qty: i.qty + 1 } : i
        ),
      });
    } else {
      setNewOrder({
        ...newOrder,
        items: [...newOrder.items, { menuId: item._id, name: item.name, price: platformPrice, qty: 1 }],
      });
    }
  };
  ...
  const handlePrintBill = (order) => {
    const restaurantName = config.restaurantName || "Apna Restaurant";
    const gstValue = Number(config.gstPercentage) || 5;
    
    // totalAmount is the NET PAYABLE (after markup and discount)
    const netPayable = Number(order.totalAmount);
    
    // Reverse calculating Taxable Value (Subtotal) from the Net Amount
    // Formula: Subtotal = Net / (1 + GST_Rate)
    const taxableValue = netPayable / (1 + (gstValue / 100));
    const gstAmount = netPayable - taxableValue;
    ...
    printWindow.document.write(`
      <html>
        ...
        <body>
          <div class="center"><h2 style="margin:0;">${restaurantName}</h2><p style="margin:2px 0;">TAX INVOICE</p></div>
          <div class="border-top">
            <p><b>Bill:</b> #${order._id.slice(-6).toUpperCase()} <span style="float:right;">${new Date(order.createdAt).toLocaleDateString()}</span></p>
            <p><b>Cust:</b> ${order.customerName}</p>
            ${order.orderType === "Delivery" ? `<p><b>Phone:</b> ${order.phone}</p>` : 
          ...
          <table>
            ...
            <tbody>
              ${order.items.map((item) => {
                const itemName = item.menuId?.name || item.name;
                // Item price here is the markup price stored during order creation
                const itemPrice = Number(item.price || item.menuId?.price || 0);
                return `<tr><td>${itemName}</td><td class="right">${item.qty}</td><td class="right">₹${(itemPrice * item.qty).toFixed(2)}</td></tr>`
              }).join("")}
            </tbody>
          </table>
          <div class="border-top">
            <div style="display:flex; justify-content:space-between; margin-bottom:2px;">
                <span>Taxable Amount:</span><span>₹${taxableValue.toFixed(2)}</span>
            </div>
            ${order.discount > 0 ? `<div style="display:flex; justify-content:space-between; color:red; margin-bottom:2px;"><span>Discount:</span><span>-₹${Number(order.discount).toFixed(2)}</span></div>` : ""}
            <div style="display:flex; justify-content:space-between; margin-bottom:4px;">
                <span>GST (${gstValue}%):</span><span>₹${gstAmount.toFixed(2)}</span>
            </div>
            <div class="border-top bold" style="display:flex; justify-content:space-between; font-size:15px; padding-top:5px;">
              <span>NET PAYABLE:</span><span>₹${netPayable.toFixed(2)}</span>
            </div>
          </div>
          <div class="center" style="margin-top:20px; font-size:10px;">
            <p>THANK YOU! VISIT AGAIN</p>
            <p>Generated by SmartMenu POS</p>
          </div>
          <script>window.onload = () => { window.print(); window.close(); };</script>
        </body>
      </html>
            ...
            <div key={status} className="bg-gray-200/50 p-4 rounded-3xl min-h-[80vh] border border-gray-200">
              ...
              <div className="space-y-5">
                {groupedOrders[status].map((order) => (
                  <div key={order._id} className={`bg-white p-5 rounded-3xl shadow-md border-t-8 transition-all ${order.orderType === "Swiggy" ? "border-t-orange-500" : order.orderType === "Zomato" ? "border-t-red-600" : order.orderType === "Delivery" ? "border-t-purple-600" : "border-t-slate-900"}`}>
                    <div className="flex justify-between items-start mb-4">
                      <span className={`px-2.5 py-1 rounded-lg text-[10px] font-black uppercase border ${order.orderType === "Swiggy" ? "bg-orange-50 text-orange-600 border-orange-100" : order.orderType === "Zomato" ? "bg-red-50 text-red-600 border-red-100" : "bg-slate-50 text-slate-700 border-slate-100"}`}>{order.orderType === "Swiggy" ? "🧡 Swiggy" : order.orderType === "Zomato" ? "❤️ Zomato" : order.orderType === "Delivery" ? "🛵 Delivery" : "🍽️ Dine-In"}</span>
                    ...
                    <h3 className="font-black text-2xl text-gray-900 mb-1 leading-none">{order.orderType === "Dining" ? `Table #${order.tableNumber}` : "Online Order"}</h3>
                    <div className="flex flex-col gap-1 mb-3 mt-1">
                      <p className="text-gray-500 text-xs font-bold italic">👤 {order.customerName}</p>
                      {order.orderType === "Delivery" && (
                        <><p className="text-purple-600 text-[11px] font-bold">📞 {order.phone}</p><p className="text-gray-400 text-[10px] leading-tight bg-gray-50 p-2 rounded-lg border border-dashed mt-1">📍 {order.address}</p></>
                      )}
                    </div>
                    ...
                    <ul className="mt-4 text-sm border-t border-dashed pt-4 space-y-2">
                      {order.items.map((item) => (
                        <li key={item._id} className="flex justify-between font-bold text-gray-800">
                          <span>{item.menuId?.name || item.name}</span> <span className="text-gray-400">x{item.qty}</span>
                        </li>
                      ))}
                    </ul>
                    ...
                    <div className="flex flex-col gap-2 mt-6">
                      ...
                      {status === "Served" && (
                        <><button onClick={() => handlePrintBill(order)} className="w-full py-3 bg-green-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg">📄 Print Final Bill</button><button onClick={() => deleteOrder(order._id)} className="w-full py-3 bg-gray-100 text-gray-400 rounded-2xl font-black text-xs uppercase">Archive</button></>
                      )}
                    </div>

                    <div id={`kot-${order._id}`} className="hidden">
                      <div className="center"><p className="bold large">{order.orderType.toUpperCase()} KOT</p><p className="bold" style={{fontSize: '32px', margin: '5px 0'}}>{order.orderType === "Dining" ? `TABLE: ${order.tableNumber}` : order.orderType}</p><p>ID: #{order._id.slice(-6).toUpperCase()}</p><div className="dashed-line"></div></div>
                      <table><thead><tr><th>ITEM</th><th style={{textAlign:'center'}}>QTY</th></tr></thead><tbody>{order.items.map(i => <tr key={i._id}><td className="bold">{i.menuId?.name || i.name}</td><td style={{textAlign:'center'}} className="bold">x{i.qty}</td></tr>)}</tbody></table>
                      <div className="dashed-line"></div><p className="bold">Cust: {order.customerName}</p>
                      {order.orderType === "Delivery" && (<div style={{fontSize: '12px'}}><p><b>Phone:</b> ${order.phone}</p><p><b>Address:</b> ${order.address}</p></div>)}
              ...
              <div className="w-2/5 p-8 bg-gray-50/50 flex flex-col">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                        {["Dining", "Swiggy", "Zomato"].map(plat => (
                            <button key={plat} onClick={() => { setSelectedPlatform(plat); setNewOrder({...newOrder, items: []}); }} className={`py-2 rounded-xl text-[10px] font-black border-2 transition-all ${selectedPlatform === plat ? "bg-black text-white border-black" : "bg-white text-gray-400 border-gray-100"}`}>{plat}</button>
                        ))}
                    </div>
                  </div>
                  {selectedPlatform === "Dining" && (
                    <div><label className="text-[10px] font-black text-gray-400 uppercase block mb-1">Table Number</label><input type="number" placeholder="E.g. 10" className="w-full p-3 rounded-xl border-2 border-white shadow-sm font-black text-xl" value={newOrder.tableNumber} onChange={(e) => setNewOrder({...newOrder, tableNumber: e.target.value})} /></div>
                  )}
                  <div><label className="text-[10px] font-black text-red-400 uppercase block mb-1">Apply Discount (₹)</label><input type="number" placeholder="0" className="w-full p-3 rounded-xl border-2 border-red-50 text-red-600 shadow-sm font-black text-xl" value={newOrder.discount} onChange={(e) => setNewOrder({...newOrder, discount: e.target.value})} /></div>
                </div>
                <div className="flex-1 overflow-y-auto mb-6 pr-2">
                  <p className="text-[10px] font-black text-gray-400 uppercase mb-4">Current Cart</p>
                  {newOrder.items.map((item, idx) => (
                    <div key={idx} className="flex justify-between items-center mb-3 bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
                      <div><p className="text-xs font-black">{item.name}</p><p className="text-[10px] text-gray-400">₹{item.price.toFixed(2)} x {item.qty}</p></div>
                ...
                <div className="border-t-2 border-dashed pt-6">
                  <div className="flex justify-between items-end mb-6">
                    ...
                    <span className="font-black text-4xl text-gray-900">₹{Math.max(0, newOrder.items.reduce((sum, i) => sum + (i.price * i.qty), 0) - newOrder.discount).toFixed(2)}</span>
                  </div>
