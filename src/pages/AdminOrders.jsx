...
const AdminOrders = () => {
  ...
  // Added config state to store restaurant branding/GST
  ...
  useEffect(() => {
    ...
    fetchOrders();
    fetchMenu();
    fetchSettings(); // Fetch branding/GST on load
  ...
  const addItemToManual = (item) => {
    // 1. Get markup based on platform
    let markupPercent = 0;
    if (selectedPlatform === "Swiggy")
      markupPercent = config.swiggyMarkup || 20;
    if (selectedPlatform === "Zomato")
      markupPercent = config.zomatoMarkup || 25;

    // 2. Calculate the "Aggregator Price"
      ...
      setNewOrder({
        ...
        items: [
          ...newOrder.items,
          {
            menuId: item._id,
            name: item.name,
            price: platformPrice, // This is now the high rate
            qty: 1,
          },
        ],
      });
  ...
  const handlePrintBill = (order) => {
    ...
    printWindow.document.write(`
      <html>
        ...
        <body>
          <div class="center"><h2 style="margin:0;">${restaurantName}</h2><p>TAX INVOICE</p></div>
          <div class="border-top">
            ...
            ${
              order.orderType === "Delivery"
                ? `<p><b>Phone:</b> ${order.phone}</p><p><b>Addr:</b> ${order.address}</p>`
                : order.orderType === "Dining"
                  ? `<p><b>Table:</b> ${order.tableNumber}</p>`
                  : `<p><b>Platform:</b> ${order.orderType}</p>`
            }
          </div>
          <table>
          ...
          <div class="border-top">
            ...
            <div style="display:flex; justify-content:space-between;"><span>GST (${config.gstPercentage}%):</span><span>₹${gst}</span></div>
  ...
  const filteredOrders = orders.filter(
    (o) => viewFilter === "All" || o.orderType === viewFilter,
  );
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          ...
          <div className="flex gap-4 items-center">
            <button
              ...
              className="bg-green-600 text-white px-6 py-3 rounded-2xl font-black text-sm shadow-xl hover:scale-105 transition-all"
            >
              ➕ NEW POS ORDER
            </button>
            <div className="flex bg-white p-1 rounded-xl shadow-sm border border-gray-200">
              {["All", "Dining", "Delivery", "Swiggy", "Zomato"].map((type) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-[10px] font-black transition-all ${viewFilter === type ? "bg-black text-white" : "text-gray-400 hover:bg-gray-50"}`}
                >
                  {type}
                </button>
              ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {statusCategories.map((status) => (
            ...
            >
              <h2
                className={`text-sm font-black mb-6 flex justify-between items-center px-2 uppercase tracking-widest ${status === "Pending" ? "text-orange-600" : status === "Preparing" ? "text-blue-600" : "text-green-600"}`}
              >
                {status}{" "}
                <span className="bg-white text-gray-900 px-3 py-1 rounded-full text-xs italic">
                  ...
                  >
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <button
                        ...
                        className="p-2 bg-gray-50 hover:bg-gray-100 rounded-xl border text-[10px] font-bold"
                      >
                        🖨️ KOT
                      </button>
                    </div>
                    ...
                    <div className="flex flex-col gap-1 mb-3 mt-1">
                      ...
                      {order.orderType === "Delivery" && (
                        <>
                          ...
                          <p className="text-gray-400 text-[10px] leading-tight bg-gray-50 p-2 rounded-lg border border-dashed mt-1">
                            📍 {order.address}
                          </p>
                        </>
                      )}
                    ...
                    <div className="mt-4 bg-gray-50 p-2 rounded-xl border border-dashed">
                      <select
                        className="w-full bg-transparent text-[10px] font-black outline-none text-gray-400 uppercase"
                        onChange={(e) => {
                          if (e.target.value) {
                    ...
                    <div className="flex flex-col gap-2 mt-6">
                      {status === "Pending" && (
                        <button
                          ...
                          className="w-full py-3 bg-orange-500 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg"
                        >
                          Accept Order
                        </button>
                      )}
                      {status === "Preparing" && (
                        <button
                          ...
                          className="w-full py-3 bg-blue-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg"
                        >
                          Mark Served
                        </button>
                      )}
                      {status === "Served" && (
                        <>
                          <button
                            ...
                            className="w-full py-3 bg-green-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg"
                          >
                            📄 Print Final Bill
                          </button>
                          <button
                            onClick={() => deleteOrder(order._id)}
                            className="w-full py-3 bg-gray-100 text-gray-400 rounded-2xl font-black text-xs uppercase"
                          >
                            Archive
                          </button>
                        </>
                    ...
                    <div id={`kot-${order._id}`} className="hidden">
                      <div className="center">
                        <p className="bold large">
                          {order.orderType.toUpperCase()} KOT
                        </p>
                        <p
                          className="bold"
                      ...
                      <table>
                        ...
                        <tbody>
                          {order.items.map((i) => (
                            <tr key={i._id}>
                              <td className="bold">{i.menuId?.name}</td>
                              <td
            ...
            <div className="flex flex-1 overflow-hidden">
              <div className="w-3/5 p-8 overflow-y-auto border-r border-dashed">
                <input
                  ...
                  className="w-full p-4 rounded-2xl bg-gray-100 mb-6 font-bold outline-none ring-0 focus:ring-2 focus:ring-green-500/20"
                  value={searchTerm}
                ...
                <div className="grid grid-cols-2 gap-3">
                  {menuList
                    ...
                    .map((item) => (
                      <button
                        key={item._id}
                        onClick={() => addItemToManual(item)}
                        className="text-left p-4 rounded-2xl border-2 border-gray-50 hover:border-green-500 hover:bg-green-50 transition-all flex flex-col justify-between h-24"
                      >
              ...
              <div className="w-2/5 p-8 bg-gray-50/50 flex flex-col">
                <div className="space-y-4 mb-6">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {["Dining", "Swiggy", "Zomato"].map((plat) => (
                        <button
                          ...
                          className={`py-2 rounded-xl text-[10px] font-black border-2 transition-all ${selectedPlatform === plat ? "bg-black text-white border-black" : "bg-white text-gray-400 border-gray-100"}`}
                        >
                          {plat}
                        </button>
                      ))}
                  ...
                  {selectedPlatform === "Dining" && (
                    <div>
                      ...
                      <input
                        type="number"
                        placeholder="E.g. 10"
                        className="w-full p-3 rounded-xl border-2 border-white shadow-sm font-black text-xl outline-none"
                  ...
                  <div>
                    <label className="text-[10px] font-black text-red-400 uppercase tracking-widest block mb-1">
                      Apply Discount (₹)
                    </label>
                    <input
                      type="number"
                      placeholder="0"
                      className="w-full p-3 rounded-xl border-2 border-red-50 text-red-600 shadow-sm font-black text-xl outline-none"
                ...
                <div className="flex-1 overflow-y-auto mb-6 pr-2">
                  ...
                  {newOrder.items.map((item) => (
                    <div
                      key={item.menuId}
                      className="flex justify-between items-center mb-3 bg-white p-4 rounded-2xl shadow-sm border border-gray-100"
                    >
                      <div>
                ...
                <div className="border-t-2 border-dashed pt-6">
                  ...
                  <button
                    ...
                    className="w-full py-5 bg-green-600 text-white rounded-[24px] font-black uppercase tracking-widest shadow-xl hover:bg-green-700 transition-all"
                  >
                    🚀 Generate Bill & KOT
                  </button>
                </div>
